...
//    }
   // tgt->timestamp = anl->timestamp; // absolut time stamp
    tgt->tpat = anl->atpat; // tpat
    tgt->ptrig = anl->aptrig; // physics trigger
}
...
void TFRSUserProc::Process_Analysis(TFRSSortEvent &srt, TFRSCalibrEvent &cal, TFRSAnlEvent &anl, TFRSUserEvent &tgt) 
{
  ...
  for(int i = 0; i<10; i++)
    {
      tgt.mrtof_stop[i] = anl.mrtof_stop[i];
      tgt.mrtof_tof[i] = anl.mrtof_tof[i];
    }
